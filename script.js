...
function selectRole(btn, role) {
    currentRole = role;
    btn.closest('.role-pills').querySelectorAll('.role-pill').forEach(p => p.classList.remove('active'));
...
function doLogin() {
    showNotif('Welcome back! Redirecting to dashboard...');
    setTimeout(() => {
...
function setupDashboard() {
    ...
    const initials = currentRole === 'Dentist' ? 'DS' : 'PA';
    document.getElementById('sidebar-avatar').textContent = initials;
    document.getElementById('sidebar-name').textContent = currentRole === 'Dentist' ? 'Dr. Smith' : 'Alex Patient';
    ...
    if (currentRole === 'Dentist') {
      ...
      <div class="grid-2">
        <div class="card">
          ...
          <table class="tbl">
            ...
            <tbody>
              <tr><td>09:00</td><td>Sara Ahmed</td><td><span class="badge badge-blue">Checkup</span></td><td><button class="btn-sm" onclick="navigate('teleconsult')">Join</button></td></tr>
              <tr><td>10:30</td><td>James Malik</td><td><span class="badge badge-amber">X-ray</span></td><td><button class="btn-sm" onclick="navigate('analysis')">Analyze</button></td></tr>
              <tr><td>14:00</td><td>Priya Singh</td><td><span class="badge badge-green">Follow-up</span></td><td><button class="btn-sm" onclick="navigate('teleconsult')">Join</button></td></tr>
            </tbody>
          </table>
        </div>
        <div class="card">
          <h3>Recent AI findings</h3>
          <div class="ai-finding"><div class="ai-finding-dot" style="background:#e24b4a"></div><div><div style="font-size:13px;font-weight:500">Caries detected — Tooth #14</div><div style="font-size:11px;color:var(--color-text-secondary)">Patient: James Malik · 92% confidence</div></div></div>
          <div class="ai-finding"><div class="ai-finding-dot" style="background:#ef9f27"></div><div><div style="font-size:13px;font-weight:500">Early bone loss — lower molar</div><div style="font-size:11px;color:var(--color-text-secondary)">Patient: Sara Ahmed · 87% confidence</div></div></div>
          <div class="ai-finding"><div class="ai-finding-dot" style="background:#639922"></div><div><div style="font-size:13px;font-weight:500">No abnormalities found</div><div style="font-size:11px;color:var(--color-text-secondary)">Patient: Priya Singh · 96% confidence</div></div></div>
        </div>
      </div>`;
      ...
      <div class="grid-2">
        <div class="card"><h3>Latest report</h3>
          <div class="ai-finding"><div class="ai-finding-dot" style="background:#ef9f27"></div><div><div style="font-size:13px;font-weight:500">Mild caries — tooth #14</div><div style="font-size:11px;color:var(--color-text-secondary)">Reviewed by Dr. Smith · Apr 12</div></div></div>
        ...
        <div class="card"><h3>Upcoming</h3>
          <div class="timeline-item"><div class="timeline-dot"></div><div><div style="font-size:13px;font-weight:500">Video consultation</div><div style="font-size:11px;color:var(--color-text-secondary)">May 5 · 10:00 AM · Dr. Smith</div></div></div>
...
function reportModal() {
    ...
    <div class="field"><label>Patient name</label><input type="text" value="James Malik"/></div>
...
function renderPatients() {
    ...
    <div class="card">
      ...
      <table class="tbl">
        ...
        <tbody>
          ${[
            ['Sara Ahmed','32','Apr 12, 2025','Active'],
            ['James Malik','45','Mar 28, 2025','Pending'],
            ['Priya Singh','28','Apr 20, 2025','Active'],
            ['Omar Hassan','52','Feb 14, 2025','Inactive'],
            ['Lucy Chen','39','Apr 25, 2025','Active']
          ].map(([n,a,d,s]) => `
          <tr>
    ...
    <div class="card" style="margin-top:1rem">
      <h3>Patient timeline — Sara Ahmed</h3>
      ${[
    ...
    <div class="grid-2">
      ...
      <div class="card">
        ...
        <table class="tbl">
          ...
          <tbody>
            <tr><td>May 3</td><td>Sara Ahmed</td><td><span class="badge badge-blue">Checkup</span></td><td><button class="btn-sm" onclick="showNotif('Reminder sent')">Remind</button></td></tr>
            <tr><td>May 8</td><td>James Malik</td><td><span class="badge badge-amber">Analysis</span></td><td><button class="btn-sm" onclick="showNotif('Reminder sent')">Remind</button></td></tr>
            <tr><td>May 14</td><td>Priya Singh</td><td><span class="badge badge-green">Follow-up</span></td><td><button class="btn-sm" onclick="showNotif('Reminder sent')">Remind</button></td></tr>
          </tbody>
        </table>
...
function bookAptModal(slot) {
    ...
    <div style="background:var(--color-background-secondary);border-radius:8px;padding:.75rem;margin-bottom:1rem;font-size:13px">
      ...
      <div><strong>Dentist:</strong> Dr. Smith</div>
    </div>
...
function renderTeleconsult() {
          ...
          <div style="background:#0d1117;border-radius:var(--border-radius-lg) var(--border-radius-lg) 0 0;position:relative">
            <div style="width:100%;aspect-ratio:16/9;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#1a1a2e,#0d1b2a)">
              <div style="text-align:center">
                <div class="avatar" style="width:64px;height:64px;font-size:22px;margin:0 auto 8px">DS</div>
                <div style="color:rgba(255,255,255,.8);font-size:14px;font-weight:500">Dr. Smith</div>
            ...
            <div style="position:absolute;bottom:12px;right:12px;width:100px;aspect-ratio:4/3;background:linear-gradient(135deg,#0f3460,#533483);border-radius:8px;border:2px solid rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center">
              <div style="text-align:center">
                <div class="avatar" style="width:28px;height:28px;font-size:10px;margin:0 auto 4px">PA</div>
                <div style="color:rgba(255,255,255,.7);font-size:10px">You</div>
              </div>
            </div>
            <div style="position:absolute;top:10px;left:12px;background:rgba(0,0,0,.5);color:white;font-size:12px;padding:3px 8px;border-radius:20px" id="call-timer">00:00</div>
          </div>
    ...
    if (timerEl) {
        const iv = setInterval(() => {
            if (!document.getElementById('call-timer')) { clearInterval(iv); return; }
            secs++;
...
function endCallModal() {
    ...
    <p style="font-size:13px;color:var(--color-text-secondary);margin-bottom:1rem">The session has been 4:23. A summary will be saved to the patient record.</p>
    <div style="display:flex;gap:8px">
      <button class="btn-sm" style="background:#e24b4a;color:white;border-color:#e24b4a;flex:1" onclick="closeModal();navigate('patients');showNotif('Call ended. Summary saved.')">End call</button>
...
function renderReports() {
    ...
    <div class="card">
      <table class="tbl">
        ...
        <tbody>
          ${[
            ['Sara Ahmed','Apr 12','Bone loss','87%','badge-amber'],
            ['James Malik','Mar 28','Caries #14','92%','badge-red'],
            ['Priya Singh','Apr 20','No issues','96%','badge-green'],
            ['Omar Hassan','Feb 14','Calculus','80%','badge-amber'],
            ['Lucy Chen','Apr 25','No issues','98%','badge-green']
          ].map(([n,d,f,c,b])=>`
          <tr>
            <td>${n}</td><td>${d}</td>
...
function renderMyReports() {
    ...
    <div class="card">
      ...
      ${[
        ['Apr 12, 2025','Mild caries — tooth #14','Reviewed by Dr. Smith','badge-amber'],
...
function renderSubscription() {
    document.getElementById('main-content').innerHTML = `
    <div class="topbar"><h1>Subscription plans</h1></div>
    <div class="grid-3">
      <div class="plan-card">
        <div style="font-size:14px;font-weight:500">Free</div>
        <div class="plan-price">$0<span style="font-size:14px;font-weight:400">/mo</span></div>
        <div class="plan-feature">View 2 AI reports/month</div>
        <div class="plan-feature">Basic appointment booking</div>
        <div class="plan-feature">Email support</div>
        <div class="plan-feature" style="color:var(--color-text-secondary)">No video consultations</div>
        <button class="btn-sm" style="width:100%;margin-top:1rem">Current plan</button>
      </div>
      <div class="plan-card featured">
        <div style="display:flex;align-items:center;justify-content:space-between"><div style="font-size:14px;font-weight:500">Pro</div><span class="badge badge-blue">Most popular</span></div>
        <div class="plan-price">$19<span style="font-size:14px;font-weight:400">/mo</span></div>
        <div class="plan-feature">Unlimited AI reports</div>
        <div class="plan-feature">Video consultations</div>
        <div class="plan-feature">Priority support</div>
        <div class="plan-feature">Appointment reminders</div>
        <button class="btn-sm btn-blue-sm" style="width:100%;margin-top:1rem" onclick="showNotif('Redirecting to payment...')">Upgrade to Pro</button>
      </div>
      <div class="plan-card">
        <div style="font-size:14px;font-weight:500">Family</div>
        <div class="plan-price">$39<span style="font-size:14px;font-weight:400">/mo</span></div>
        <div class="plan-feature">Everything in Pro</div>
        <div class="plan-feature">Up to 5 family members</div>
        <div class="plan-feature">Shared health records</div>
        <div class="plan-feature">Dedicated dental advisor</div>
        <button class="btn-sm" style="width:100%;margin-top:1rem" onclick="showNotif('Redirecting to payment...')">Get Family plan</button>
      </div>
    </div>`;
}
